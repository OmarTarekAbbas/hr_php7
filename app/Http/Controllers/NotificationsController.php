...
class NotificationsController extends Controller {
    ...
    function getUpdate(Request $request, $id = null) {
        ...
        if ($row) {
            ...
            $this->data['row'] = $this->model->getColumnTable('tb_notifications');
        }


        $this->data['id'] = $id;
        return view('notifications.form', $this->data);
    }
    ...
    public function getShow($id = null) {
        ...
        $this->data['id'] = $id;
        $this->data['access'] = $this->access;
        return view('notifications.view', $this->data);
    }
    ...
    function postSave(Request $request) {
        ...
        if ($validator->passes()) {
            $data = $this->validatePost('tb_notifications');

            $id = $this->model->insertRow3($data, $request->input('id'));

            if (!is_null($request->input('apply'))) {
                $return = 'notifications/update/' . $id . '?return=' . self::returnUrl();
            } else {
                $return = 'notifications?return=' . self::returnUrl();
            }
            ...
            return Redirect::to('notifications/update/' . $id)->with('messagetext', \Lang::get('core.note_error'))->with('msgstatus', 'error')
    ...
    function postSeennotification() {
        $id = Input::get('id');
        \DB::table('tb_notifications')
                ->where('id', $id)
                ->update(['seen' => 1]);
    }

    public static function postUpnotifications() {
        ...
        //  echo "<li><a href='" . \URL::to('notifications') . "' > " .\ Lang::get('core.view_all_notifications') . "</a></li>";
        echo "</ul>";
    }
