...
class SiteHelpers {
    ...
    public static function addNotification($notifier_id, $notified_id, $subject, $link) {
        ...
        \DB::table('tb_notifications')->insert($data);
      ...
      //  $request = \Illuminate\Http\Request::capture();
        ...
        $message_link = \URL::to('/') . DIRECTORY_SEPARATOR . $link;
...
// echo 'From: ' . $Notifier->email . ' TO:  '.$Notified->email; die ;
    ...
    public static function addNotificationDelay($notifier_id, $notified_id, $subject, $link, $delay_mesage) {
        $hr_email = delay_notifications_email;
        date_default_timezone_set("Africa/Cairo");
        $data = array(
            'notifier_id' => $notifier_id,
            'notified_id' => $notified_id,
        ...
        // to get request on the fly
        $request = \Illuminate\Http\Request::capture();
        $message_link = $request->root() . DIRECTORY_SEPARATOR . $link;
        // send mail
        $message = '<!DOCTYPE html>
					<html lang="en-US">
						<head>
...
// echo 'From: ' . $Notifier->email . ' TO:  '.$Notified->email; die ;
    ...
    public static function signcheck()
    {

        $currentUserId =Auth::user()->id;
        $currentDate = date('Y-m-d') ;
        $attendance_sign_check  = Attendance::where('employee_id',Auth::user()->id)->where('date',date('Y-m-d'))->first() ;
        //  print_r($attendance) ; die;
        if($attendance_sign_check){
            if($attendance_sign_check->sign_in != "" && $attendance_sign_check->sign_out != ""  ){
                $sign_in = 0 ;
                $sign_out = 0 ;
            }elseif ($attendance_sign_check->sign_out == ""){
                $sign_in = 0 ;
                $sign_out = 1 ;
            }else{
                $sign_in = 0 ;
        ...
        return  ['sign_in'=>$sign_in, 'sign_out'=>$sign_out ]  ;


    }


    public static function networkcheck()
    {
        $result = 0 ;
        $URL = 'http://10.2.10.10/~hrivashosting/' ;  // real link =   $URL = 'http://hr.ivashosting.com/dashboard'
        ...
        if (empty($buffer)){
            $result = 0;  // login outside  OR Nothing returned from url
        }
        else{
            $result =  1;  // login inside
        }
        return $result ;
    }
