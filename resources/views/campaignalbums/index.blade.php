...
<div class="sbox-tools" >
	...
	<div class="sbox-content">
	    <div class="toolbar-line ">
			@if($access['is_add'] ==1)
	   		<a href="{{ URL::to('campaignalbums/update') }}" class="tips btn btn-sm btn-white"  title="{{ Lang::get('core.btn_create') }}">
			...
			<a href="{{ URL::to('campaignalbums/download') }}" class="tips btn btn-sm btn-white" title="{{ Lang::get('core.btn_download') }}">
	 ...
	 {!! Form::open(array('url'=>'campaignalbums/delete/', 'class'=>'form-horizontal' ,'id' =>'SximoTable' )) !!}
        ...
        <td></td>
         ...
         <td><a  href="{{ URL::to('playtrack_v4/'.$row->id.'?return='.$return) }}" class="tips btn btn-xs btn-white" title="Show  Tracks">Show Track</a></td>
				 <td>
						...
						<a href="{{ URL::to('campaignalbums/show/'.$row->id.'?return='.$return)}}" class="tips btn btn-xs btn-white" title="{{ Lang::get('core.btn_view') }}"><i class="fa  fa-search "></i></a>
						@endif
						@if($access['is_edit'] ==1)
...
$(document).ready(function(){
	...
	$('.do-quick-search').click(function(){
		$('#SximoTable').attr('action','{{ URL::to("campaignalbums/multisearch")}}');
		$('#SximoTable').submit();
	});
