    ...
    <div class="page-content-wrapper">
        ...
        <div class="sbox animated fadeInRight">
            ...
            <div class="sbox-content"> 	
                ...
                {!! Form::open(array('url'=>'permissions/save?return='.$return, 'class'=>'form-horizontal','files' => true , 'parsley-validate'=>'','novalidate'=>' ')) !!}
                <div class="col-md-12">
                    <fieldset><legend> permissions</legend>
                      ...
                      <div class="form-group" >
                            ...
                            <div class="col-md-6 emp_vacation_margin">
                                 {{ $row->from }}
                                 <input type="hidden" name="from" value="{{$row->from}}"  >
                            </div> 
                            <div class="col-md-2">
                        ...
                        <div class="form-group" >
                            <label for="Date" class=" control-label col-md-4 text-left"> Hours : </label>
                            <div class="col-md-6 emp_vacation_margin">
                                {{ $row->hours }}
                            </div> 
                            <div class="col-md-2">
                ...
                <div class="form-group">
                    ...
                    <!--<button type="submit" name="apply" class="btn btn-info btn-sm" ><i class="fa  fa-check-circle"></i> {{ Lang::get('core.sb_apply') }}</button>-->
                        ...
                        <button type="button" onclick="location.href ='{{ URL::to('permissions?return='.$return) }}' " class="btn btn-success btn-sm "><i class="fa  fa-arrow-circle-left "></i>  {{ Lang::get('core.sb_cancel') }} </button>
                    </div>	  
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $("#employee_id").jCombo("{{ URL::to('permissions/comboselect?filter=tb_users:id:first_name|last_name') }}",
                {selected_value: '{{ $row["employee_id"] }}'});

        $("#department_id").jCombo("{{ URL::to('permissions/comboselect?filter=tb_departments:id:title') }}",
                {selected_value: '{{ $row["department_id"] }}'});

        $("#manager_id").jCombo("{{ URL::to('permissions/comboselect?filter=tb_users:id:first_name|last_name') }}",
                {selected_value: '{{ $row["manager_id"] }}'});


        $("#from").jCombo("{{ URL::to('permissions/comboselect?filter=tb_permissions_hours:id:value') }}",
                {selected_value: '{{ $row["from"] }}'});

    $("#to").jCombo("{{ URL::to('permissions/comboselect?filter=tb_permissions_hours:id:value') }}",
                {selected_value: '{{ $row["to"] }}'});
